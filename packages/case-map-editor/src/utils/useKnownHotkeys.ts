...
export const useKnownHotkeys = () => {
  ...
  const addStage = useMemo<KnownHotkey>(() => {
    const hotkey = 'A';
    return { hotkey, label: t('hotkey.addStage', { hotkey: hotkeyText(hotkey) }) };
  }, [t]);

  const deleteStage = useMemo<KnownHotkey>(() => {
    const hotkey = 'Delete';
    return { hotkey, label: t('hotkey.deleteStage', { hotkey: hotkeyText(hotkey) }) };
  }, [t]);
  ...
  return { undo, redo, addStage, deleteStage, focusToolbar, focusMain, focusInscription, openHelp };
};
